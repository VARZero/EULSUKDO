...
**[아키텍쳐 생성기 스크린샷]**
![현재 Gen 아키텍처 생성기](gen/generator-app/docs_img/gen_core_current.png)
...
RTL만 보면 지금 무엇이 만들어지는지 감이 잘 안 올 수 있죠. 그래서 두 웹앱도 넣었습니다.

- [아키텍쳐 생성기](gen/generator-app/README.md)는 디코드 폭, 레지스터 수, EX 경로와 ISA 명령을 정하면 현재 설정의 top·디코더와 RTL 전체를 ZIP으로 내려받습니다.
- [시뮬레이션 비주얼라이저](gen/sim_visualizer/README.md)는 테스트벤치가 만든 VCD를 읽어서 모듈별 신호와 파형을 사이클 단위로 보여줍니다. 먼저 들어 있는 데모 VCD로 둘러볼 수도 있어요.

두 앱의 실행 방법과 아직 구현되지 않은 범위는 [여기](gen/WEB_APPS.md)에 적어 두었습니다.
...
gen/ : 을숙도 아키텍쳐 커스텀 도구에 관련된 디렉토리입니다. 
  generator-app/ : 현재 RTL에 맞는 구조·ISA 생성기입니다.
  sim_visualizer/ : 스케줄러 VCD 파형을 표시하는 웹앱입니다.
...
## 이 프로젝트는 순수 개인의 설계와 LLM을 통한 구조 확장이 혼합된 레포지토리 입니다.
직접 작성한 부분은
- ```docs/``` 디렉토리 내부의 문서는 직접 모두 작성하였습니다.
