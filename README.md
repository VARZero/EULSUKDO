...
알고리즘의 주요 컨셉인 **명령의 상태를 이용**하는 방법을 내부적인 회로로 나누는 구조를 적용해봤습니다.  

## 구현 컨셉
토마슬로 알고리즘이 추구하는 **레지스터 이름 변경**을 적용한  
**비순차 실행 처리를 진행하는 동적 스케줄링 구조**를 만드는것이 최종적인 목표입니다.
이때, Fanout 증가 문제와 전력 소모를 줄이기 위해 CAM이라는 구조를 배제하여 구현했습니다.
...
Google Gemini 모델로 Antigravity를 이용하여 작성된 부분은
- ```gen/``` 디렉토리 내부의 작업물들 입니다.
    - ```gen/src/``` 디렉토리 내부의 코드는  ```docs/```의 내용을 제약사항으로 걸고 ```structure_src/```를 참조하여  
    확장 가능한 형태로 작성하도록 유도하였고, 제가 적극적으로 코드를 수정하며 개입하였습니다.
    - ```gen/generator-app/```의 생성기 코드는 ```docs/```의 내용을 제약사항으로 걸고 
    기본 레이아웃 구성, 세부적인 메뉴 구성을 중점을 프롬프트로 전달하였습니다.

현재 ```gen/```부분에 RTL코드에 문제가 있어, ```gen2/```에 직접 리팩토링을 진행하고 있습니다.
