...
아래 **[복사하여 사용할 프롬프트]** 전체를 새 Codex 채팅에 붙여 넣고, `~/work/EULSUKDO` 작업 디렉터리을 열어 준다. `ISA/uop 표`가 정해지지 않았다면 첫 단계는 인터페이스와 기본 ALU/BR 골격까지만 구현하도록 둔다.

---

## [복사하여 사용할 프롬프트]

당신은 EULSUKDO `gen` 작업본의 **실행부(EX)**를 설계·구현하는 SystemVerilog 개발자다. 먼저 다음 파일을 실제로 읽고 신호·필드 위치를 확인하라.
...
- `gen/generator-app/src/utils/rtlGenerator.ts`, `gen/generator-app/src/utils/decoderGenerator.ts`: 생성 코드의 EX 연결 위치와 ISA-uop 정의.
...
기본 범위는 **정수 ALU + 분기/점프 EX + 물리 레지스터 데이터 저장/읽기 + EX-WBC 연결**이다. 메모리 EX는 별도 경로와 구체적인 메모리 계약을 정의한 후 확장한다. 첫 단계의 성공은 임의의 연산 코드에 대한 형식상 `done`이 아니라, 두 소스 레지스터의 실제 값 읽기, 진짜 결과값, 종속 명령 wakeup, PC 완료/분기 전환까지 통합해서 확인하는 것이다.
...
1. 실제 포트/패킷/디코더-uop 표를 추출한 뒤 `ALU / BR / MEM`별 지원 연산을 표로 확정한다. 미정 uop는 임의 할당하지 않는다. gen 생성기의 초기 ADD/SUB/LW는 예제이며 완전한 RV32I 정의가 아니다.
